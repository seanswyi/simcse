...
class TRECEval(object):
    ...
    def run(self, params, batcher):
        ...
        sorted_corpus_train = sorted(
            zip(self.train["X"], self.train["y"]), key=lambda z: (len(z[0]), z[1])
        )
        train_samples = [x for (x, y) in sorted_corpus_train]
        train_labels = [y for (x, y) in sorted_corpus_train]

        sorted_corpus_test = sorted(
            zip(self.test["X"], self.test["y"]), key=lambda z: (len(z[0]), z[1])
        )
