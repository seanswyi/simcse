...
class SNLIEval(object):
    def __init__(self, taskpath, seed=1111):
        ...
        trainlabels = (
            io.open(os.path.join(taskpath, "labels.train"), encoding="utf-8")
            .read()
            .splitlines()
        )
        ...
        validlabels = (
            io.open(os.path.join(taskpath, "labels.dev"), encoding="utf-8")
            .read()
            .splitlines()
        )
        ...
        testlabels = (
            io.open(os.path.join(taskpath, "labels.test"), encoding="utf-8")
            .read()
            .splitlines()
        )
        ...
        sorted_train = sorted(
            zip(train2, train1, trainlabels), key=lambda z: (len(z[0]), len(z[1]), z[2])
        )
        train2, train1, trainlabels = map(list, zip(*sorted_train))

        sorted_valid = sorted(
            zip(valid2, valid1, validlabels), key=lambda z: (len(z[0]), len(z[1]), z[2])
        )
        valid2, valid1, validlabels = map(list, zip(*sorted_valid))

        sorted_test = sorted(
            zip(test2, test1, testlabels), key=lambda z: (len(z[0]), len(z[1]), z[2])
        )
        ...
        for key in self.data:
            ...
            for ii in range(0, n_labels, params.batch_size):
                ...
                if len(batch1) == len(batch2) and len(batch1) > 0:
                    ...
                    enc_input.append(
                        np.hstack((enc1, enc2, enc1 * enc2, np.abs(enc1 - enc2)))
                    )
