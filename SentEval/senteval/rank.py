...
class ImageCaptionRetrievalEval(object):
    ...
    def loadFile(self, fpath):
        ...
        for split in ["train", "valid", "test"]:
            ...
            for imgkey in range(len(cocodata["features"])):
                assert len(cocodata["image_to_caption_ids"][imgkey]) >= 5, cocodata[
                    "image_to_caption_ids"
                ][imgkey]
    ...
    def run(self, params, batcher):
        ...
        for key in self.coco_data:
            ...
            ), np.argsort(self.coco_data[key]["sent"])
            ...
            coco_embed[key]["sentfeat"] = np.vstack(coco_embed[key]["sentfeat"])[
                idx_unsort
            ]
