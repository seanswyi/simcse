...
class SE(object):
    ...
    def eval(self, name):
        ...
        if name == "CR":
            ...
            self.evaluation = MPQAEval(
                tpath + "/downstream/MPQA", seed=self.params.seed
            )
        elif name == "SUBJ":
            self.evaluation = SUBJEval(
                tpath + "/downstream/SUBJ", seed=self.params.seed
            )
        elif name == "SST2":
            self.evaluation = SSTEval(
                tpath + "/downstream/SST/binary", nclasses=2, seed=self.params.seed
            )
        elif name == "SST5":
            self.evaluation = SSTEval(
                tpath + "/downstream/SST/fine", nclasses=5, seed=self.params.seed
            )
        elif name == "TREC":
            self.evaluation = TRECEval(
                tpath + "/downstream/TREC", seed=self.params.seed
            )
        elif name == "MRPC":
            self.evaluation = MRPCEval(
                tpath + "/downstream/MRPC", seed=self.params.seed
            )
        elif name == "SICKRelatedness":
            self.evaluation = SICKRelatednessEval(
                tpath + "/downstream/SICK", seed=self.params.seed
            )
        elif name == "STSBenchmark":
            self.evaluation = STSBenchmarkEval(
                tpath + "/downstream/STS/STSBenchmark", seed=self.params.seed
            )
        elif name == "STSBenchmark-fix":
            self.evaluation = STSBenchmarkEval(
                tpath + "/downstream/STS/STSBenchmark-fix", seed=self.params.seed
            )
        elif name == "STSBenchmark-finetune":
            self.evaluation = STSBenchmarkFinetune(
                tpath + "/downstream/STS/STSBenchmark", seed=self.params.seed
            )
        elif name == "SICKRelatedness-finetune":
            self.evaluation = SICKEval(
                tpath + "/downstream/SICK", seed=self.params.seed
            )
            ...
            self.evaluation = SNLIEval(
                tpath + "/downstream/SNLI", seed=self.params.seed
            )
        elif name in ["STS12", "STS13", "STS14", "STS15", "STS16"]:
            fpath = name + "-en-test"
            self.evaluation = eval(name + "Eval")(
                tpath + "/downstream/STS/" + fpath, seed=self.params.seed
            )
        elif name == "ImageCaptionRetrieval":
            self.evaluation = ImageCaptionRetrievalEval(
                tpath + "/downstream/COCO", seed=self.params.seed
            )
        ...
        # Probing Tasks
            ...
            self.evaluation = TopConstituentsEval(
